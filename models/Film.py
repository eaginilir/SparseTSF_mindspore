...
class Model(nn.Cell):
    ...
    def construct(self, x_enc, x_mark_enc=None, x_dec=None, x_mark_dec=None,
        ...
        mean = reduce_mean(x_enc, 1).repeat((1, self.pred_len, 1))  # 重复，保持形状
...
if __name__ == '__main__':
    ...
    # 用 numpy 生成随机数，再转为 MindSpore Tensor
    enc = Tensor(np.random.randn(32, configs.seq_len, 7).astype(np.float32))
    enc_mark = Tensor(np.random.randn(32, configs.seq_len, 4).astype(np.float32))

    dec = Tensor(np.random.randn(32, configs.label_len + configs.pred_len, 7).astype(np.float32))
    dec_mark = Tensor(np.random.randn(32, configs.label_len + configs.pred_len, 4).astype(np.float32))
