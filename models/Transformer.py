...
class Model(nn.Cell):
    ...
    def __init__(self, configs):
        ...
        self.input = nn.Dense(in_channels=self.enc_in, out_channels=self.d_model)
        self.output = nn.Dense(in_channels=self.d_model, out_channels=self.enc_in)
    ...
    def construct(self, x_enc, x_mark_enc=None, x_dec=None, x_mark_dec=None,
        ...
        dec_inp = Tensor(np.zeros((batch_size, self.pred_len, self.d_model), dtype=np.float32))
        dec_inp = dec_inp.to(device=x_enc.device)
